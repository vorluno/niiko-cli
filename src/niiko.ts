...
const CATALOGO: readonly Accion[] = [
  {
...
function leerConfig(): Config | null {
  ...
  try {
    const c = JSON.parse(readFileSync(RUTA_CONFIG, "utf8")) as Partial<Config>;
...
function credenciales(args: Map<string, string>): Config | null {
...
function salir(codigo: number, cuerpo: unknown): never {
  process.stdout.write(JSON.stringify(cuerpo, null, 2) + "\n");
  process.exit(codigo);
}

/** `--a b` y `--a=b`. Lo que no empieza por `--` es posicional. */
function parsear(argv: string[]): { posicionales: string[]; args: Map<string, string> } {
...
function leerEntrada(args: Map<string, string>): unknown {
  const crudo = args.get("input");
  let texto: string;
  if (crudo === undefined) {
    if (process.stdin.isTTY) salir(1, { error: "falta la entrada: --input '{…}', --input @archivo, o por stdin" });
    texto = readFileSync(0, "utf8");
  } else if (crudo.startsWith("@")) texto = readFileSync(crudo.slice(1), "utf8");
  else texto = crudo;
  try {
...
async function main(): Promise<void> {
  ...
  if (cmd === "login") {
    ...
    if (!apiKey) salir(1, { error: "falta --key nk_… (o NIIKO_API_KEY)" });
  ...
  if (cmd === "describe") {
    ...
    if (!a) salir(1, { error: `no existe la acción \`${id}\``, disponibles: CATALOGO.map((x) => x.accion) });
    salir(0, a);
  }
  ...
  if (cmd === "run") {
    ...
    if (!cred) salir(1, { error: "sin llave: niiko login --key nk_…, o NIIKO_API_KEY, o --key" });
    ...
    const status = (cuerpo as { status?: string } | null)?.status;
    ...
    const salida = typeof cuerpo === "object" && cuerpo !== null ? { ...cuerpo, idempotencyKey: (cuerpo as { idempotencyKey?: string }).idempotencyKey ?? clave } : { http: r.status, cuerpo };
    if (status === "done") salir(0, salida);
    if (status === "pending_approval") salir(3, salida);
    if (status === "refused") salir(2, salida);
